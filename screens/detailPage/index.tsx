...
export default function DetailPage({navigation}) {
  ...
  return (
    <>
      ...
      <BackView>
        <Header>
          {!objectsDetails?.statusPicture||objectsDetails?.owner ?
        <TouchEditRight
          onPress={()=>{navigation.navigate("EditItemPage")}}>
          <IconRight resizeMode={"center"} source={Icon} />
          </TouchEditRight>
          :null}
        ...
        <Padding>
          ...
          <FlatList
            ...
            renderItem={({item}) => {
              ...
              return (
                <View style={{ width: (Dimensions.get('window').width-50)/3, height: 105 }}>
                  <ImageItem source={{uri:`${BASE_URL}/api/v1/files/${item?.id}`}} />
                </View>
              );
            }}
          />
      ...
      >
        {objectsDetails?.address[0]?.location!=null?
         <MapView.Marker
              coordinate={{
                latitude:objectsDetails?.address[0]?.location?.lng!=null ?objectsDetails?.address[0]?.location?.lat:0,
                longitude: objectsDetails?.address[0]?.location?.lat!=null ?objectsDetails?.address[0]?.location?.lng:0,
              }}
